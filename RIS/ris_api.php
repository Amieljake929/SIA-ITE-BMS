<?php
// ris_api.php - Updated to use first_name, middle_name, last_name
...
// ------------------------------
// POST: Add new resident (for RIS internal use) - UPDATED
// ------------------------------
function handlePostRequest($conn) {
    $data = json_decode(file_get_contents("php://input"), true);

    // UPDATED: Required fields now check for first_name and last_name
    $required_fields = [
        'first_name', 'last_name', 'email', 'dob', 'pob', 'age', 'gender', 'civil_status',
    ...
    // UPDATED: Insert statement now uses separate name columns
    $sql = "INSERT INTO registration (
        id, first_name, middle_name, last_name, email, dob, pob, age, gender, civil_status,
    ...
    // UPDATED: Bind parameters match the new name columns
    $stmt->bind_param(
        "sssssssisssssssisssssiiiiii",
        $new_id,
        $data['first_name'], $data['middle_name'] ?? null, $data['last_name'], 
        $data['email'], $data['dob'], $data['pob'], $data['age'],
...
// ------------------------------
// GET: Validate if resident exists by reference_number, resident_id, email, or phone - UPDATED
// ------------------------------
function handleGetRequest($conn) {
    ...
    if ($ref) {
        // UPDATED: Use CONCAT_WS to create full_name from separate name columns
        $stmt = $conn->prepare("
            SELECT CONCAT_WS(' ', r.first_name, r.middle_name, r.last_name) AS full_name, 
                   r.email, r.dob, r.phone, r.pob, r.gender, 
    ...
    if ($id) {
        ...
        // UPDATED: Use CONCAT_WS to create full_name
        $stmt2 = $conn->prepare("
            SELECT CONCAT_WS(' ', first_name, middle_name, last_name) AS full_name, 
                   email, dob, phone, pob, gender, civil_status, 
    ...
    // UPDATED: Use CONCAT_WS to create full_name
    $sql = "SELECT id, CONCAT_WS(' ', first_name, middle_name, last_name) AS full_name, email, phone, dob FROM registration WHERE ";
    $params = [];
    $types = "";

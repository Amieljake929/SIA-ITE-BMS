...
// Get archived registrations data
$sql = "
    SELECT *,
           CONCAT_WS(' ', first_name, middle_name, last_name) AS full_name
    FROM registration_archive
...
<head>
  ...
  <title>Bagbag eServices - Registration Archive</title>
  ...
  <header class="bg-white shadow-lg border-b border-green-100 px-6 py-4">
    <div class="container mx-auto flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
      <div class="flex items-center space-x-4">
        ...
        <h1 class="text-xl font-bold text-green-800">Registration Archive</h1>
      </div>
            ...
            <?php if ($result && $result->num_rows > 0): ?>
              <?php while($row = $result->fetch_assoc()): ?>
                <tr class="hover:bg-gray-50" data-name="<?= htmlspecialchars(strtolower($row['full_name'])) ?>"
                  ...
                  <td class="px-4 py-2 text-xs text-gray-500"><?= htmlspecialchars($row['dob']) ?></td>
                  <td class="px-4 py-2 text-xs text-gray-500"><?= htmlspecialchars($row['rejected_at']) ?></td>
                  <td class="px-4 py-2 max-w-xs">
                    <div class="truncate" title="<?= htmlspecialchars($row['remarks']) ?>">
                      <?= htmlspecialchars(substr($row['remarks'], 0, 50)) ?><?php if (strlen($row['remarks']) > 50): ?>...<?php endif; ?>
                    </div>
                  </td>
                  <td class="px-4 py-2">
                    <a href="ris_archive_details.php?id=<?= urlencode($row['id']) ?>"

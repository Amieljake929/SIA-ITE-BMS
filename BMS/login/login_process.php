...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            // Check if role is resident for MFA
            if (strtolower(trim($role)) === 'resident') {
                // Check for existing unexpired MFA code for this resident
                $check_stmt = $conn->prepare("SELECT code, expires_at FROM resident_mfa WHERE resident_id = ? AND expires_at > NOW() LIMIT 1");
                ...
                if ($check_stmt->num_rows > 0) {
                    ...
                    $mfa_stmt = $conn->prepare("INSERT INTO resident_mfa (resident_id, code, verified, expires_at) VALUES (?, ?, 0, ?)");
                ...
                $subject = "Your 2FA Code for Barangay Bagbag Login";
                $body = "
                <html>
                <head>
                    <style>
                ...
                <body>
                    <div class='container'>
                        <h2 class='header'>Barangay Bagbag 2FA Verification</h2>
                ...
                $_SESSION['mfa_resident_id'] = $id; // Assuming resident_id is same as user_id, adjust if different
                ...
                // For non-resident roles, proceed as before
                $_SESSION['user_id'] = $id;

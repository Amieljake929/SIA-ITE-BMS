...
$stmt_check = $conn->prepare("SELECT id, status, cedula_id FROM cedula WHERE user_id = ? AND status IN ('Pending', 'Validated', 'Approved') ORDER BY application_date DESC LIMIT 1");
...
$stmt_user = $conn->prepare("
    SELECT u.full_name, u.email, r.dob, r.pob, r.civil_status, r.address, r.phone
    FROM users u
...
<head>
  ...
  <title>Community Tax Certificate (Cedula) Application</title>
...
<body class="bg-gray-50 text-gray-800 font-sans flex flex-col min-h-screen">
  ...
  <header class="bg-white shadow-lg border-b border-green-100 px-6 py-4">
    <div class="container mx-auto flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
        <div class="flex items-center space-x-4">
            ...
            <h1 class="text-xl font-bold text-green-800">Community Tax Certificate (Cedula) Application</h1>
        </div>
  ...
  <main class="flex-grow px-6 py-8">
    <div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-100">
      ...
      <h2 class="text-2xl font-bold text-green-800 mb-6 text-center">Community Tax Certificate (Cedula)</h2>
      <p class="text-gray-600 text-center mb-8 text-sm">Please provide accurate information to apply for your Community Tax Certificate.</p>
      ...
      <?php if ($active_request && $active_request['status'] === 'Approved'): ?>
        <div class="bg-green-50 border-l-4 border-green-400 p-6 mb-8 rounded-lg">
          <div class="flex items-start">
            ...
            <div class="ml-4">
              <h3 class="text-lg font-semibold text-green-800">Cedula Approved!</h3>
              <p class="text-green-700 mt-1">Your Community Tax Certificate has been <strong>approved</strong>.</p>
              <p class="text-sm text-green-600 mt-2">
                <strong>📄 Cedula ID:</strong> <?= htmlspecialchars($active_request['cedula_id']) ?><br>
        ...
        <div class="text-center space-y-4">
          <a href="print_document.php?tab=cedula&id=<?= $active_request['id'] ?>" target="_blank"
            ...
            <i class="fas fa-print mr-2"></i> Print Cedula
          </a>
          <div>
      ...
      <?php else: ?>
        <form id="cedulaForm" action="R.submit_cedula.php" method="POST">
          ...
          <section class="mb-8">
            <h3 class="text-lg font-semibold text-gray-800 mb-4 border-b pb-2 border-green-200">Cedula Information</h3>
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 mb-1">Occupation / Business</label>
              <input type="text" name="occupation" placeholder="e.g., Teacher, Driver, Vendor" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:border-green-500 transition" required>
            </div>
            <div class="mb-4">
          ...
          <div class="text-center">
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-semibold px-8 py-3 rounded-lg shadow transition duration-200 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-green-300">
              <i class="fas fa-file-alt mr-2"></i> Submit Cedula Request
            </button>
          </div>
        </form>
      <?php endif; ?>
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      if (form) {
        ...
        form.addEventListener('submit', function(e) {
          ...
          if (!confirm("Are you sure you want to submit your Cedula request?")) {
            e.preventDefault();
          }
        });
      }
    ...
    if (urlParams.has('success') && urlParams.get('success') === 'cedula') {
      alert("✅ Success! Your Cedula request has been submitted.\n\nStatus: Pending Approval\nYou will be notified once it's ready.");

...
$stmt_check = $conn->prepare("SELECT id, status, business_permit_id FROM business_permit WHERE user_id = ? AND status IN ('Pending', 'Validated', 'Approved') ORDER BY application_date DESC LIMIT 1");
...
<body class="bg-gray-50 text-gray-800 font-sans flex flex-col min-h-screen">
        ...
        <form id="businessPermitForm" action="R.submit_business_permit.php" method="POST" enctype="multipart/form-data">
          ...
          <section class="mb-8">
            <h3 class="text-lg font-semibold text-gray-800 mb-4 border-b pb-2 border-green-200 flex items-center">
              <i class="fas fa-user mr-2 text-green-600"></i> Personal Information (Non-Editable)
            </h3>
  ...
  <script>
    ...
    // Auto-fill today's date and handle dropdown

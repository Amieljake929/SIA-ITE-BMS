...
<body class="bg-gray-50 text-gray-800 font-sans flex flex-col min-h-screen">
    ...
    <div class="container mx-auto flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
      ...
      <div class="relative inline-block text-right">
        <button 
          id="userMenuButton" 
          type="button"
          class="flex items-center font-medium cursor-pointer text-sm focus:outline-none whitespace-nowrap hover:text-blue-800 transition-colors duration-200"
        >
          <span class="text-gray-800">Logged in:</span>
          <span class="text-blue-700 ml-1"><?php echo htmlspecialchars($_SESSION['full_name']); ?></span>
          <i class="fas fa-chevron-down ml-2 text-gray-400 transform transition-transform duration-200" id="dropdownIcon"></i>
        </button>
        <div 
          id="userDropdown" 
          class="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-xl opacity-0 invisible transform scale-95 transition-all duration-200 z-50"
        >
  ...
  <main class="flex-grow px-6 py-8">
    <div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-100">
      ...
      <h2 class="text-2xl font-bold text-green-800 mb-6 text-center">First-Time Job Seeker Certificate</h2>
      ...
      <?php if ($active_request && $active_request['status'] === 'Approved'): ?>
        <div class="bg-green-50 border-l-4 border-green-400 p-6 mb-8 rounded-lg">
          <div class="flex items-start">
            ...
            <div class="ml-4">
              ...
              <p class="text-green-700 mt-1">Your First-Time Job Seeker Certificate has been <strong>approved</strong>.</p>
              <p class="text-sm text-green-600 mt-2">
                <strong>📄 Certificate ID:</strong> <?= htmlspecialchars($active_request['first_time_job_id']) ?><br>
        ...
        <div class="text-center space-y-4">
          <a href="print_document.php?tab=first_time_job_seekers&id=<?= $active_request['id'] ?>" target="_blank"
          ...
          <section class="mb-8">
            ...
            <div class="bg-gray-50 p-4 rounded border border-gray-200 mb-4">
              <p class="text-sm text-gray-700">
                I hereby declare that I am a first-time job seeker and I have not been employed before.
              </p>
            </div>
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      if (form) {
        ...
        form.addEventListener('submit', function(e) {
          ...
          if (!confirm("Are you sure you want to submit your First-Time Job Seeker request?")) {
            e.preventDefault();
          }
        });
      }

      // Dropdown functionality with improved handling
    ...
    if (urlParams.has('success') && urlParams.get('success') === 'first_time_job') {
      alert("✅ Success! Your First-Time Job Seeker request has been submitted.\n\nStatus: Pending Approval\nYou will be notified once it's ready.");

...
// Fetch reports that are ready for closing
$sql = "SELECT cr.*, 
               u.full_name as bpso_name,
               CONCAT(cr.first_name, ' ', cr.last_name) as resident_name
        FROM community_reports cr
        LEFT JOIN users u ON cr.assigned_to = u.id
        WHERE cr.status = 'For Closing'
...
<head>
  ...
  <title>Close Reports - Bagbag eServices</title>
  <!-- Tailwind CSS -->
...
<body class="bg-gray-50 text-gray-800 font-sans flex flex-col min-h-screen">
  ...
  <main class="container mx-auto px-6 py-8 flex-grow">
    <h2 class="text-2xl font-semibold text-gray-700 mb-6">Reports for Final Review</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-green-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-green-800 uppercase">BPSO</th>
          ...
          <tbody class="divide-y divide-gray-200">
            <?php while ($row = $result->fetch_assoc()): ?>
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 text-sm"><?= htmlspecialchars($row['resident_name']) ?></td>
                <td class="px-6 py-4 text-sm"><?= htmlspecialchars($row['incident_type']) ?></td>
                <td class="px-6 py-4 text-sm"><?= htmlspecialchars($row['bpso_name'] ?? 'Not Assigned') ?></td>
    ...
    <?php else: ?>
      <p class="text-gray-500 text-center py-8">Walang reports na kailangan i-close sa ngayon.</p>
    <?php endif; ?>
  </main>
